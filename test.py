...
def endgame_afternoon(data, not_ww_index):
    # Check on noon
    count = 0
    num_not_ww = len(not_ww_index)
...
displayname = ['KingBabelac', 'Buabelac', 'TikusBerdasi', 'Cumi', 'Lele', 'Kevin']
# WW - Seer - Villager
...
while True:
    ...
    choice_to_kill = input('Pick your choices (WEREWOLF): ') # pm
    print('Not WW Index: ' + str(not_ww_index))

    for k in range(len(not_ww_index)):
    ...
    if data[seer_index]['status'] == True: # Check if seer is still alive, else not pm him
        ...
        choice_to_peek = input('Pick your choices (SEER): ') # pm
    ...
    vote_input = input('Who are you guys going to vote? ') # pm group ## CHECK THE MOST COUNTS IN THE VOTING 
    ...
    time.sleep(2)
    endgame_afternoon(data, not_ww_index)
